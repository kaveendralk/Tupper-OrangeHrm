...
Feature: Validate that user is able to create Non party orders
  ...
  Background: Validate non party order creation
    ...
    And User clicks on "Sales" on "Home" page
  ...
  @CreateOrder @NonPartyOrder @NonPartyOrder_AddProduct @TC_10 @Smoke
    ...
    And User adds multiple product of different product catagory to the cart
    And  User verify the added product of mutiple category
 ...
 #And User verify that correct product is added by Search
    #And User clicks on "Browse Products" on "Create Order" page
